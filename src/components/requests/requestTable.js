    ...
    return (
      <div className="mainTable">
        <table className="mainTable_table">
          ...
          {this.props.requests && this.props.requests.map(request => (<tr key={request.id} className="mainTable_table_tr">
            ...
            <td className="mainTable_table_td">
              ...
              disabled={request.status !== 'Pending'}
                onClick={() => this.props.togglePopup(request)}>Edit</button></td>
          </tr>))}
        </table>
      </div>
    );
